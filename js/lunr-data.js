window.lunrData = {
  "index": {
    "version": "1.0.0",
    "fields": [
      {
        "name": "longname",
        "boost": 1000
      },
      {
        "name": "name",
        "boost": 500
      },
      {
        "name": "tags",
        "boost": 300
      },
      {
        "name": "kind",
        "boost": 110
      },
      {
        "name": "title",
        "boost": 100
      },
      {
        "name": "summary",
        "boost": 70
      },
      {
        "name": "description",
        "boost": 50
      },
      {
        "name": "body",
        "boost": 1
      }
    ],
    "ref": "id",
    "tokenizer": "default",
    "documentStore": {
      "store": {
        "index.html": [
          "build",
          "chrome",
          "cli",
          "command",
          "develop",
          "document",
          "extens",
          "index",
          "line",
          "project",
          "provid",
          "readm",
          "speed",
          "systemat",
          "test",
          "tool",
          "up",
          "util",
          "way"
        ],
        "global.html": [
          "document",
          "global"
        ],
        "undefined": [
          "old_readm",
          "tutori"
        ],
        "list_tutorial.html": [
          "cli",
          "extens",
          "guid",
          "learn",
          "list",
          "list:tutori",
          "us"
        ],
        "module-xt-build.html": [
          "browser",
          "build",
          "call",
          "chrome",
          "command",
          "compil",
          "debug",
          "directori",
          "dist",
          "distribut",
          "e",
          "env",
          "file",
          "flag",
          "gener",
          "minifi",
          "modul",
          "module:xt",
          "prod",
          "product",
          "release.zip",
          "store",
          "upload",
          "web",
          "xt"
        ],
        "module-xt-clean.html": [
          "clean",
          "delet",
          "directori",
          "each",
          "except",
          "explicitli",
          "file",
          "flag",
          "gitignor",
          "idea",
          "ignor",
          "iter",
          "list",
          "modul",
          "module:xt",
          "node_modul",
          "on",
          "oper",
          "over",
          "pass",
          "project",
          "remov",
          "vscode",
          "xt"
        ],
        "module-xt-create.html": [
          "command",
          "creat",
          "extens",
          "implement",
          "modul",
          "module:xt",
          "new",
          "onc",
          "project",
          "us",
          "xt"
        ],
        "module-xt-docs.html": [
          "add",
          "altern",
          "automat",
          "c",
          "chang",
          "command",
          "config",
          "configur",
          "default",
          "defin",
          "detail",
          "doc",
          "document",
          "explicitli",
          "file",
          "flag",
          "foodoc",
          "gener",
          "here",
          "includ",
          "jsdoc",
          "key",
          "locat",
          "look",
          "modul",
          "module:xt",
          "more",
          "name",
          "option",
          "opts.templ",
          "overrid",
          "package.json",
          "path",
          "project",
          "provid",
          "root",
          "see",
          "separ",
          "syntax",
          "templat",
          "theme",
          "us",
          "want",
          "xt",
          "xtdoc",
          "xtdocs.json"
        ],
        "module-xt-sync.html": [
          "advis",
          "cli",
          "command",
          "config",
          "configur",
          "file",
          "heavili",
          "instead",
          "latest",
          "manner",
          "manual",
          "modifi",
          "modul",
          "module:xt",
          "project",
          "provid",
          "purpos",
          "stale",
          "such",
          "sync",
          "tool",
          "updat",
          "upgrad",
          "version",
          "xt"
        ],
        "module-xt-test.html": [
          "api",
          "arrow",
          "babel",
          "base",
          "browser",
          "chai",
          "chrome",
          "class",
          "code",
          "command",
          "comput",
          "coverag",
          "creat",
          "directori",
          "environ",
          "es6",
          "execut",
          "expect",
          "extend",
          "extens",
          "follow",
          "global",
          "import",
          "includ",
          "initi",
          "jsdom",
          "locat",
          "mean",
          "mocha",
          "modul",
          "module:xt",
          "nyc",
          "pre",
          "project",
          "promis",
          "run",
          "setup",
          "simpli",
          "singl",
          "sinon",
          "suitabl",
          "test",
          "unit",
          "us",
          "window",
          "xt"
        ]
      },
      "length": 10
    },
    "tokenStore": {
      "root": {
        "docs": {},
        "b": {
          "docs": {},
          "u": {
            "docs": {},
            "i": {
              "docs": {},
              "l": {
                "docs": {},
                "d": {
                  "docs": {
                    "index.html": {
                      "ref": "index.html",
                      "tf": 4.117647058823529
                    },
                    "undefined": {
                      "tf": 964
                    },
                    "module-xt-build.html": {
                      "ref": "module-xt-build.html",
                      "tf": 952.1739130434783
                    }
                  }
                }
              }
            }
          },
          "r": {
            "docs": {},
            "o": {
              "docs": {},
              "w": {
                "docs": {},
                "s": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "r": {
                      "docs": {
                        "module-xt-build.html": {
                          "ref": "module-xt-build.html",
                          "tf": 2.1739130434782608
                        },
                        "module-xt-test.html": {
                          "ref": "module-xt-test.html",
                          "tf": 0.7246376811594203
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "b": {
              "docs": {},
              "e": {
                "docs": {},
                "l": {
                  "docs": {
                    "module-xt-test.html": {
                      "ref": "module-xt-test.html",
                      "tf": 0.7246376811594203
                    }
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "e": {
                "docs": {
                  "module-xt-test.html": {
                    "ref": "module-xt-test.html",
                    "tf": 0.7246376811594203
                  }
                }
              }
            }
          }
        },
        "c": {
          "docs": {
            "module-xt-docs.html": {
              "ref": "module-xt-docs.html",
              "tf": 0.7246376811594203
            }
          },
          "h": {
            "docs": {},
            "r": {
              "docs": {},
              "o": {
                "docs": {},
                "m": {
                  "docs": {},
                  "e": {
                    "docs": {
                      "index.html": {
                        "ref": "index.html",
                        "tf": 4.117647058823529
                      },
                      "module-xt-build.html": {
                        "ref": "module-xt-build.html",
                        "tf": 2.1739130434782608
                      },
                      "module-xt-test.html": {
                        "ref": "module-xt-test.html",
                        "tf": 2.1739130434782608
                      }
                    }
                  }
                }
              }
            },
            "a": {
              "docs": {},
              "n": {
                "docs": {},
                "g": {
                  "docs": {
                    "module-xt-docs.html": {
                      "ref": "module-xt-docs.html",
                      "tf": 0.7246376811594203
                    }
                  },
                  "e": {
                    "docs": {},
                    "l": {
                      "docs": {},
                      "o": {
                        "docs": {},
                        "g": {
                          "docs": {
                            "undefined": {
                              "tf": 1900
                            }
                          }
                        }
                      }
                    }
                  }
                }
              },
              "i": {
                "docs": {
                  "module-xt-test.html": {
                    "ref": "module-xt-test.html",
                    "tf": 1.4492753623188406
                  }
                }
              }
            }
          },
          "l": {
            "docs": {},
            "i": {
              "docs": {
                "index.html": {
                  "ref": "index.html",
                  "tf": 300
                },
                "undefined": {
                  "tf": 25
                },
                "list_tutorial.html": {
                  "ref": "list_tutorial.html",
                  "tf": 17.5
                },
                "module-xt-sync.html": {
                  "ref": "module-xt-sync.html",
                  "tf": 1.7241379310344827
                }
              }
            },
            "e": {
              "docs": {},
              "a": {
                "docs": {},
                "n": {
                  "docs": {
                    "undefined": {
                      "tf": 950
                    },
                    "module-xt-clean.html": {
                      "ref": "module-xt-clean.html",
                      "tf": 951.9230769230769
                    }
                  }
                }
              }
            },
            "a": {
              "docs": {},
              "s": {
                "docs": {},
                "s": {
                  "docs": {
                    "module-xt-test.html": {
                      "ref": "module-xt-test.html",
                      "tf": 0.7246376811594203
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "m": {
              "docs": {},
              "m": {
                "docs": {},
                "a": {
                  "docs": {},
                  "n": {
                    "docs": {},
                    "d": {
                      "docs": {
                        "index.html": {
                          "ref": "index.html",
                          "tf": 4.117647058823529
                        },
                        "undefined": {
                          "tf": 11.666666666666666
                        },
                        "module-xt-build.html": {
                          "ref": "module-xt-build.html",
                          "tf": 4.3478260869565215
                        },
                        "module-xt-create.html": {
                          "ref": "module-xt-create.html",
                          "tf": 6.25
                        },
                        "module-xt-docs.html": {
                          "ref": "module-xt-docs.html",
                          "tf": 2.1739130434782608
                        },
                        "module-xt-sync.html": {
                          "ref": "module-xt-sync.html",
                          "tf": 1.7241379310344827
                        },
                        "module-xt-test.html": {
                          "ref": "module-xt-test.html",
                          "tf": 1.4492753623188406
                        }
                      }
                    }
                  }
                }
              },
              "p": {
                "docs": {},
                "i": {
                  "docs": {},
                  "l": {
                    "docs": {
                      "module-xt-build.html": {
                        "ref": "module-xt-build.html",
                        "tf": 2.1739130434782608
                      }
                    }
                  }
                },
                "u": {
                  "docs": {},
                  "t": {
                    "docs": {
                      "module-xt-test.html": {
                        "ref": "module-xt-test.html",
                        "tf": 0.7246376811594203
                      }
                    }
                  }
                }
              }
            },
            "n": {
              "docs": {},
              "f": {
                "docs": {},
                "i": {
                  "docs": {},
                  "g": {
                    "docs": {
                      "undefined": {
                        "tf": 17.5
                      },
                      "module-xt-docs.html": {
                        "ref": "module-xt-docs.html",
                        "tf": 2.898550724637681
                      },
                      "module-xt-sync.html": {
                        "ref": "module-xt-sync.html",
                        "tf": 3.4482758620689653
                      }
                    },
                    "u": {
                      "docs": {},
                      "r": {
                        "docs": {
                          "undefined": {
                            "tf": 11.666666666666666
                          },
                          "module-xt-docs.html": {
                            "ref": "module-xt-docs.html",
                            "tf": 3.6231884057971016
                          },
                          "module-xt-sync.html": {
                            "ref": "module-xt-sync.html",
                            "tf": 3.4482758620689653
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "d": {
              "docs": {},
              "e": {
                "docs": {
                  "undefined": {
                    "tf": 17.5
                  },
                  "module-xt-test.html": {
                    "ref": "module-xt-test.html",
                    "tf": 0.7246376811594203
                  }
                }
              }
            },
            "o": {
              "docs": {},
              "l": {
                "docs": {
                  "undefined": {
                    "tf": 11.666666666666666
                  }
                }
              }
            },
            "v": {
              "docs": {},
              "e": {
                "docs": {},
                "r": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "g": {
                      "docs": {
                        "module-xt-test.html": {
                          "ref": "module-xt-test.html",
                          "tf": 0.7246376811594203
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "r": {
            "docs": {},
            "e": {
              "docs": {},
              "a": {
                "docs": {},
                "t": {
                  "docs": {
                    "undefined": {
                      "tf": 950
                    },
                    "module-xt-create.html": {
                      "ref": "module-xt-create.html",
                      "tf": 956.25
                    },
                    "module-xt-test.html": {
                      "ref": "module-xt-test.html",
                      "tf": 0.7246376811594203
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "l": {
              "docs": {},
              "l": {
                "docs": {
                  "module-xt-build.html": {
                    "ref": "module-xt-build.html",
                    "tf": 2.1739130434782608
                  }
                }
              }
            }
          }
        },
        "d": {
          "docs": {},
          "e": {
            "docs": {},
            "v": {
              "docs": {
                "undefined": {
                  "tf": 14
                }
              },
              "e": {
                "docs": {},
                "l": {
                  "docs": {},
                  "o": {
                    "docs": {},
                    "p": {
                      "docs": {
                        "index.html": {
                          "ref": "index.html",
                          "tf": 4.117647058823529
                        }
                      }
                    }
                  }
                }
              }
            },
            "t": {
              "docs": {},
              "a": {
                "docs": {},
                "i": {
                  "docs": {},
                  "l": {
                    "docs": {
                      "undefined": {
                        "tf": 25
                      },
                      "module-xt-docs.html": {
                        "ref": "module-xt-docs.html",
                        "tf": 0.7246376811594203
                      }
                    }
                  }
                }
              }
            },
            "b": {
              "docs": {},
              "u": {
                "docs": {},
                "g": {
                  "docs": {
                    "module-xt-build.html": {
                      "ref": "module-xt-build.html",
                      "tf": 2.1739130434782608
                    }
                  }
                }
              }
            },
            "l": {
              "docs": {},
              "e": {
                "docs": {},
                "t": {
                  "docs": {
                    "module-xt-clean.html": {
                      "ref": "module-xt-clean.html",
                      "tf": 1.9230769230769231
                    }
                  }
                }
              }
            },
            "f": {
              "docs": {},
              "a": {
                "docs": {},
                "u": {
                  "docs": {},
                  "l": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "module-xt-docs.html": {
                          "ref": "module-xt-docs.html",
                          "tf": 1.4492753623188406
                        }
                      }
                    }
                  }
                }
              },
              "i": {
                "docs": {},
                "n": {
                  "docs": {
                    "module-xt-docs.html": {
                      "ref": "module-xt-docs.html",
                      "tf": 1.4492753623188406
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "c": {
              "docs": {
                "undefined": {
                  "tf": 967.5
                },
                "module-xt-docs.html": {
                  "ref": "module-xt-docs.html",
                  "tf": 951.4492753623189
                }
              },
              "u": {
                "docs": {},
                "m": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "n": {
                      "docs": {},
                      "t": {
                        "docs": {
                          "index.html": {
                            "ref": "index.html",
                            "tf": 4.117647058823529
                          },
                          "global.html": {
                            "ref": "global.html",
                            "tf": 35
                          },
                          "module-xt-docs.html": {
                            "ref": "module-xt-docs.html",
                            "tf": 0.7246376811594203
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "i": {
            "docs": {},
            "r": {
              "docs": {},
              "e": {
                "docs": {},
                "c": {
                  "docs": {},
                  "t": {
                    "docs": {},
                    "o": {
                      "docs": {},
                      "r": {
                        "docs": {},
                        "i": {
                          "docs": {
                            "undefined": {
                              "tf": 14
                            },
                            "module-xt-build.html": {
                              "ref": "module-xt-build.html",
                              "tf": 2.1739130434782608
                            },
                            "module-xt-clean.html": {
                              "ref": "module-xt-clean.html",
                              "tf": 5.769230769230769
                            },
                            "module-xt-test.html": {
                              "ref": "module-xt-test.html",
                              "tf": 0.7246376811594203
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "t": {
                "docs": {
                  "module-xt-build.html": {
                    "ref": "module-xt-build.html",
                    "tf": 2.1739130434782608
                  }
                },
                "r": {
                  "docs": {},
                  "i": {
                    "docs": {},
                    "b": {
                      "docs": {},
                      "u": {
                        "docs": {},
                        "t": {
                          "docs": {
                            "module-xt-build.html": {
                              "ref": "module-xt-build.html",
                              "tf": 2.1739130434782608
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "e": {
          "docs": {
            "module-xt-build.html": {
              "ref": "module-xt-build.html",
              "tf": 2.1739130434782608
            }
          },
          "x": {
            "docs": {},
            "t": {
              "docs": {},
              "e": {
                "docs": {},
                "n": {
                  "docs": {},
                  "s": {
                    "docs": {
                      "index.html": {
                        "ref": "index.html",
                        "tf": 308.2352941176471
                      },
                      "undefined": {
                        "tf": 14
                      },
                      "list_tutorial.html": {
                        "ref": "list_tutorial.html",
                        "tf": 17.5
                      },
                      "module-xt-create.html": {
                        "ref": "module-xt-create.html",
                        "tf": 6.25
                      },
                      "module-xt-test.html": {
                        "ref": "module-xt-test.html",
                        "tf": 0.7246376811594203
                      }
                    }
                  },
                  "d": {
                    "docs": {
                      "module-xt-test.html": {
                        "ref": "module-xt-test.html",
                        "tf": 0.7246376811594203
                      }
                    }
                  }
                }
              }
            },
            "c": {
              "docs": {},
              "e": {
                "docs": {},
                "p": {
                  "docs": {},
                  "t": {
                    "docs": {
                      "module-xt-clean.html": {
                        "ref": "module-xt-clean.html",
                        "tf": 1.9230769230769231
                      }
                    }
                  }
                }
              }
            },
            "p": {
              "docs": {},
              "l": {
                "docs": {},
                "i": {
                  "docs": {},
                  "c": {
                    "docs": {},
                    "i": {
                      "docs": {},
                      "t": {
                        "docs": {},
                        "l": {
                          "docs": {},
                          "i": {
                            "docs": {
                              "module-xt-clean.html": {
                                "ref": "module-xt-clean.html",
                                "tf": 1.9230769230769231
                              },
                              "module-xt-docs.html": {
                                "ref": "module-xt-docs.html",
                                "tf": 0.7246376811594203
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              },
              "e": {
                "docs": {},
                "c": {
                  "docs": {},
                  "t": {
                    "docs": {
                      "module-xt-test.html": {
                        "ref": "module-xt-test.html",
                        "tf": 0.7246376811594203
                      }
                    }
                  }
                }
              }
            },
            "e": {
              "docs": {},
              "c": {
                "docs": {},
                "u": {
                  "docs": {},
                  "t": {
                    "docs": {
                      "module-xt-test.html": {
                        "ref": "module-xt-test.html",
                        "tf": 0.7246376811594203
                      }
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "c": {
              "docs": {},
              "h": {
                "docs": {
                  "undefined": {
                    "tf": 17.5
                  },
                  "module-xt-clean.html": {
                    "ref": "module-xt-clean.html",
                    "tf": 1.9230769230769231
                  }
                }
              }
            }
          },
          "n": {
            "docs": {},
            "v": {
              "docs": {
                "module-xt-build.html": {
                  "ref": "module-xt-build.html",
                  "tf": 2.1739130434782608
                }
              },
              "i": {
                "docs": {},
                "r": {
                  "docs": {},
                  "o": {
                    "docs": {},
                    "n": {
                      "docs": {
                        "module-xt-test.html": {
                          "ref": "module-xt-test.html",
                          "tf": 2.898550724637681
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "s": {
            "6": {
              "docs": {
                "module-xt-test.html": {
                  "ref": "module-xt-test.html",
                  "tf": 0.7246376811594203
                }
              }
            },
            "docs": {}
          }
        },
        "i": {
          "docs": {},
          "n": {
            "docs": {},
            "d": {
              "docs": {},
              "e": {
                "docs": {},
                "x": {
                  "docs": {
                    "index.html": {
                      "ref": "index.html",
                      "tf": 1300
                    }
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "t": {
                "docs": {},
                "a": {
                  "docs": {},
                  "l": {
                    "docs": {
                      "undefined": {
                        "tf": 11.666666666666666
                      }
                    }
                  }
                },
                "e": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "d": {
                      "docs": {
                        "module-xt-sync.html": {
                          "ref": "module-xt-sync.html",
                          "tf": 1.7241379310344827
                        }
                      }
                    }
                  }
                }
              }
            },
            "c": {
              "docs": {},
              "l": {
                "docs": {},
                "u": {
                  "docs": {},
                  "d": {
                    "docs": {
                      "module-xt-docs.html": {
                        "ref": "module-xt-docs.html",
                        "tf": 0.7246376811594203
                      },
                      "module-xt-test.html": {
                        "ref": "module-xt-test.html",
                        "tf": 0.7246376811594203
                      }
                    }
                  }
                }
              }
            },
            "i": {
              "docs": {},
              "t": {
                "docs": {},
                "i": {
                  "docs": {
                    "module-xt-test.html": {
                      "ref": "module-xt-test.html",
                      "tf": 1.4492753623188406
                    }
                  }
                }
              }
            }
          },
          "d": {
            "docs": {},
            "e": {
              "docs": {},
              "a": {
                "docs": {
                  "module-xt-clean.html": {
                    "ref": "module-xt-clean.html",
                    "tf": 1.9230769230769231
                  }
                }
              }
            }
          },
          "g": {
            "docs": {},
            "n": {
              "docs": {},
              "o": {
                "docs": {},
                "r": {
                  "docs": {
                    "module-xt-clean.html": {
                      "ref": "module-xt-clean.html",
                      "tf": 1.9230769230769231
                    }
                  }
                }
              }
            }
          },
          "t": {
            "docs": {},
            "e": {
              "docs": {},
              "r": {
                "docs": {
                  "module-xt-clean.html": {
                    "ref": "module-xt-clean.html",
                    "tf": 1.9230769230769231
                  }
                }
              }
            }
          },
          "m": {
            "docs": {},
            "p": {
              "docs": {},
              "l": {
                "docs": {},
                "e": {
                  "docs": {},
                  "m": {
                    "docs": {},
                    "e": {
                      "docs": {},
                      "n": {
                        "docs": {},
                        "t": {
                          "docs": {
                            "module-xt-create.html": {
                              "ref": "module-xt-create.html",
                              "tf": 6.25
                            }
                          }
                        }
                      }
                    }
                  }
                }
              },
              "o": {
                "docs": {},
                "r": {
                  "docs": {},
                  "t": {
                    "docs": {
                      "module-xt-test.html": {
                        "ref": "module-xt-test.html",
                        "tf": 0.7246376811594203
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "l": {
          "docs": {},
          "i": {
            "docs": {},
            "n": {
              "docs": {},
              "e": {
                "docs": {
                  "index.html": {
                    "ref": "index.html",
                    "tf": 4.117647058823529
                  },
                  "undefined": {
                    "tf": 11.666666666666666
                  }
                }
              },
              "k": {
                "docs": {
                  "undefined": {
                    "tf": 950
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "t": {
                "docs": {
                  "list_tutorial.html": {
                    "ref": "list_tutorial.html",
                    "tf": 110
                  },
                  "module-xt-clean.html": {
                    "ref": "module-xt-clean.html",
                    "tf": 1.9230769230769231
                  }
                },
                ":": {
                  "docs": {},
                  "t": {
                    "docs": {},
                    "u": {
                      "docs": {},
                      "t": {
                        "docs": {},
                        "o": {
                          "docs": {},
                          "r": {
                            "docs": {},
                            "i": {
                              "docs": {
                                "list_tutorial.html": {
                                  "ref": "list_tutorial.html",
                                  "tf": 1300
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "e": {
            "docs": {},
            "a": {
              "docs": {},
              "r": {
                "docs": {},
                "n": {
                  "docs": {
                    "undefined": {
                      "tf": 17.5
                    },
                    "list_tutorial.html": {
                      "ref": "list_tutorial.html",
                      "tf": 17.5
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "c": {
              "docs": {},
              "a": {
                "docs": {},
                "t": {
                  "docs": {
                    "module-xt-docs.html": {
                      "ref": "module-xt-docs.html",
                      "tf": 0.7246376811594203
                    },
                    "module-xt-test.html": {
                      "ref": "module-xt-test.html",
                      "tf": 0.7246376811594203
                    }
                  }
                }
              }
            },
            "o": {
              "docs": {},
              "k": {
                "docs": {
                  "module-xt-docs.html": {
                    "ref": "module-xt-docs.html",
                    "tf": 0.7246376811594203
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "t": {
              "docs": {},
              "e": {
                "docs": {},
                "s": {
                  "docs": {},
                  "t": {
                    "docs": {
                      "module-xt-sync.html": {
                        "ref": "module-xt-sync.html",
                        "tf": 1.7241379310344827
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "p": {
          "docs": {},
          "r": {
            "docs": {},
            "o": {
              "docs": {},
              "j": {
                "docs": {},
                "e": {
                  "docs": {},
                  "c": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "index.html": {
                          "ref": "index.html",
                          "tf": 4.117647058823529
                        },
                        "undefined": {
                          "tf": 961.6666666666666
                        },
                        "module-xt-clean.html": {
                          "ref": "module-xt-clean.html",
                          "tf": 1.9230769230769231
                        },
                        "module-xt-create.html": {
                          "ref": "module-xt-create.html",
                          "tf": 6.25
                        },
                        "module-xt-docs.html": {
                          "ref": "module-xt-docs.html",
                          "tf": 2.898550724637681
                        },
                        "module-xt-sync.html": {
                          "ref": "module-xt-sync.html",
                          "tf": 5.172413793103448
                        },
                        "module-xt-test.html": {
                          "ref": "module-xt-test.html",
                          "tf": 1.4492753623188406
                        }
                      }
                    }
                  }
                }
              },
              "v": {
                "docs": {},
                "i": {
                  "docs": {},
                  "d": {
                    "docs": {
                      "index.html": {
                        "ref": "index.html",
                        "tf": 4.117647058823529
                      },
                      "module-xt-docs.html": {
                        "ref": "module-xt-docs.html",
                        "tf": 1.4492753623188406
                      },
                      "module-xt-sync.html": {
                        "ref": "module-xt-sync.html",
                        "tf": 1.7241379310344827
                      }
                    }
                  }
                }
              },
              "d": {
                "docs": {
                  "undefined": {
                    "tf": 14
                  },
                  "module-xt-build.html": {
                    "ref": "module-xt-build.html",
                    "tf": 2.1739130434782608
                  }
                },
                "u": {
                  "docs": {},
                  "c": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "module-xt-build.html": {
                          "ref": "module-xt-build.html",
                          "tf": 2.1739130434782608
                        }
                      }
                    }
                  }
                }
              },
              "m": {
                "docs": {},
                "i": {
                  "docs": {},
                  "s": {
                    "docs": {
                      "module-xt-test.html": {
                        "ref": "module-xt-test.html",
                        "tf": 0.7246376811594203
                      }
                    }
                  }
                }
              }
            },
            "e": {
              "docs": {
                "module-xt-test.html": {
                  "ref": "module-xt-test.html",
                  "tf": 0.7246376811594203
                }
              }
            }
          },
          "l": {
            "docs": {},
            "a": {
              "docs": {},
              "n": {
                "docs": {
                  "undefined": {
                    "tf": 70
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "s": {
              "docs": {},
              "s": {
                "docs": {
                  "module-xt-clean.html": {
                    "ref": "module-xt-clean.html",
                    "tf": 1.9230769230769231
                  }
                }
              }
            },
            "c": {
              "docs": {},
              "k": {
                "docs": {},
                "a": {
                  "docs": {},
                  "g": {
                    "docs": {},
                    "e": {
                      "docs": {},
                      ".": {
                        "docs": {},
                        "j": {
                          "docs": {},
                          "s": {
                            "docs": {},
                            "o": {
                              "docs": {},
                              "n": {
                                "docs": {
                                  "module-xt-docs.html": {
                                    "ref": "module-xt-docs.html",
                                    "tf": 1.4492753623188406
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "t": {
              "docs": {},
              "h": {
                "docs": {
                  "module-xt-docs.html": {
                    "ref": "module-xt-docs.html",
                    "tf": 1.4492753623188406
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "r": {
              "docs": {},
              "p": {
                "docs": {},
                "o": {
                  "docs": {},
                  "s": {
                    "docs": {
                      "module-xt-sync.html": {
                        "ref": "module-xt-sync.html",
                        "tf": 1.7241379310344827
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "r": {
          "docs": {},
          "e": {
            "docs": {},
            "a": {
              "docs": {},
              "d": {
                "docs": {},
                "m": {
                  "docs": {
                    "index.html": {
                      "ref": "index.html",
                      "tf": 110
                    }
                  }
                }
              }
            },
            "m": {
              "docs": {},
              "o": {
                "docs": {},
                "v": {
                  "docs": {
                    "undefined": {
                      "tf": 14
                    },
                    "module-xt-clean.html": {
                      "ref": "module-xt-clean.html",
                      "tf": 3.8461538461538463
                    }
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "o": {
                "docs": {},
                "u": {
                  "docs": {},
                  "r": {
                    "docs": {},
                    "c": {
                      "docs": {
                        "undefined": {
                          "tf": 100
                        }
                      }
                    }
                  }
                }
              }
            },
            "l": {
              "docs": {},
              "e": {
                "docs": {},
                "a": {
                  "docs": {},
                  "s": {
                    "docs": {
                      "undefined": {
                        "tf": 35
                      }
                    },
                    "e": {
                      "docs": {},
                      ".": {
                        "docs": {},
                        "z": {
                          "docs": {},
                          "i": {
                            "docs": {},
                            "p": {
                              "docs": {
                                "module-xt-build.html": {
                                  "ref": "module-xt-build.html",
                                  "tf": 2.1739130434782608
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "n": {
              "docs": {
                "undefined": {
                  "tf": 23.333333333333332
                },
                "module-xt-test.html": {
                  "ref": "module-xt-test.html",
                  "tf": 1.4492753623188406
                }
              }
            }
          },
          "o": {
            "docs": {},
            "o": {
              "docs": {},
              "t": {
                "docs": {
                  "module-xt-docs.html": {
                    "ref": "module-xt-docs.html",
                    "tf": 0.7246376811594203
                  }
                }
              }
            }
          }
        },
        "s": {
          "docs": {},
          "p": {
            "docs": {},
            "e": {
              "docs": {},
              "e": {
                "docs": {},
                "d": {
                  "docs": {
                    "index.html": {
                      "ref": "index.html",
                      "tf": 4.117647058823529
                    }
                  }
                }
              }
            }
          },
          "y": {
            "docs": {},
            "s": {
              "docs": {},
              "t": {
                "docs": {},
                "e": {
                  "docs": {},
                  "m": {
                    "docs": {},
                    "a": {
                      "docs": {},
                      "t": {
                        "docs": {
                          "index.html": {
                            "ref": "index.html",
                            "tf": 4.117647058823529
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "n": {
              "docs": {},
              "c": {
                "docs": {
                  "undefined": {
                    "tf": 950
                  },
                  "module-xt-sync.html": {
                    "ref": "module-xt-sync.html",
                    "tf": 950
                  }
                },
                "h": {
                  "docs": {},
                  "r": {
                    "docs": {},
                    "o": {
                      "docs": {},
                      "n": {
                        "docs": {
                          "undefined": {
                            "tf": 17.5
                          }
                        }
                      }
                    }
                  }
                }
              },
              "t": {
                "docs": {},
                "a": {
                  "docs": {},
                  "x": {
                    "docs": {
                      "module-xt-docs.html": {
                        "ref": "module-xt-docs.html",
                        "tf": 0.7246376811594203
                      }
                    }
                  }
                }
              }
            }
          },
          "t": {
            "docs": {},
            "a": {
              "docs": {},
              "r": {
                "docs": {},
                "t": {
                  "docs": {
                    "undefined": {
                      "tf": 961.6666666666666
                    }
                  }
                }
              },
              "l": {
                "docs": {},
                "e": {
                  "docs": {
                    "module-xt-sync.html": {
                      "ref": "module-xt-sync.html",
                      "tf": 1.7241379310344827
                    }
                  }
                }
              }
            },
            "o": {
              "docs": {},
              "r": {
                "docs": {},
                "e": {
                  "docs": {
                    "module-xt-build.html": {
                      "ref": "module-xt-build.html",
                      "tf": 2.1739130434782608
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "u": {
              "docs": {},
              "r": {
                "docs": {},
                "c": {
                  "docs": {
                    "undefined": {
                      "tf": 17.5
                    }
                  }
                }
              }
            }
          },
          "h": {
            "docs": {},
            "o": {
              "docs": {},
              "w": {
                "docs": {},
                "c": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "s": {
                      "docs": {
                        "undefined": {
                          "tf": 950
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "e": {
            "docs": {},
            "e": {
              "docs": {
                "module-xt-docs.html": {
                  "ref": "module-xt-docs.html",
                  "tf": 0.7246376811594203
                }
              }
            },
            "p": {
              "docs": {},
              "a": {
                "docs": {},
                "r": {
                  "docs": {
                    "module-xt-docs.html": {
                      "ref": "module-xt-docs.html",
                      "tf": 0.7246376811594203
                    }
                  }
                }
              }
            },
            "t": {
              "docs": {},
              "u": {
                "docs": {},
                "p": {
                  "docs": {
                    "module-xt-test.html": {
                      "ref": "module-xt-test.html",
                      "tf": 2.1739130434782608
                    }
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "c": {
              "docs": {},
              "h": {
                "docs": {
                  "module-xt-sync.html": {
                    "ref": "module-xt-sync.html",
                    "tf": 1.7241379310344827
                  }
                }
              }
            },
            "i": {
              "docs": {},
              "t": {
                "docs": {},
                "a": {
                  "docs": {},
                  "b": {
                    "docs": {},
                    "l": {
                      "docs": {
                        "module-xt-test.html": {
                          "ref": "module-xt-test.html",
                          "tf": 0.7246376811594203
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "i": {
            "docs": {},
            "m": {
              "docs": {},
              "p": {
                "docs": {},
                "l": {
                  "docs": {},
                  "i": {
                    "docs": {
                      "module-xt-test.html": {
                        "ref": "module-xt-test.html",
                        "tf": 0.7246376811594203
                      }
                    }
                  }
                }
              }
            },
            "n": {
              "docs": {},
              "g": {
                "docs": {},
                "l": {
                  "docs": {
                    "module-xt-test.html": {
                      "ref": "module-xt-test.html",
                      "tf": 0.7246376811594203
                    }
                  }
                }
              },
              "o": {
                "docs": {},
                "n": {
                  "docs": {
                    "module-xt-test.html": {
                      "ref": "module-xt-test.html",
                      "tf": 0.7246376811594203
                    }
                  }
                }
              }
            }
          }
        },
        "t": {
          "docs": {},
          "e": {
            "docs": {},
            "s": {
              "docs": {},
              "t": {
                "docs": {
                  "index.html": {
                    "ref": "index.html",
                    "tf": 4.117647058823529
                  },
                  "undefined": {
                    "tf": 973.3333333333334
                  },
                  "module-xt-test.html": {
                    "ref": "module-xt-test.html",
                    "tf": 955.7971014492754
                  }
                }
              }
            },
            "m": {
              "docs": {},
              "p": {
                "docs": {},
                "l": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "module-xt-docs.html": {
                          "ref": "module-xt-docs.html",
                          "tf": 0.7246376811594203
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "o": {
              "docs": {},
              "l": {
                "docs": {
                  "index.html": {
                    "ref": "index.html",
                    "tf": 4.117647058823529
                  },
                  "undefined": {
                    "tf": 11.666666666666666
                  },
                  "module-xt-sync.html": {
                    "ref": "module-xt-sync.html",
                    "tf": 1.7241379310344827
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "t": {
              "docs": {},
              "o": {
                "docs": {},
                "r": {
                  "docs": {},
                  "i": {
                    "docs": {
                      "undefined": {
                        "tf": 110
                      }
                    }
                  }
                }
              }
            }
          },
          "h": {
            "docs": {},
            "e": {
              "docs": {},
              "m": {
                "docs": {},
                "e": {
                  "docs": {
                    "module-xt-docs.html": {
                      "ref": "module-xt-docs.html",
                      "tf": 0.7246376811594203
                    }
                  }
                }
              }
            }
          }
        },
        "u": {
          "docs": {},
          "p": {
            "docs": {
              "index.html": {
                "ref": "index.html",
                "tf": 4.117647058823529
              }
            },
            "l": {
              "docs": {},
              "o": {
                "docs": {},
                "a": {
                  "docs": {},
                  "d": {
                    "docs": {
                      "module-xt-build.html": {
                        "ref": "module-xt-build.html",
                        "tf": 2.1739130434782608
                      }
                    }
                  }
                }
              }
            },
            "d": {
              "docs": {},
              "a": {
                "docs": {},
                "t": {
                  "docs": {
                    "module-xt-sync.html": {
                      "ref": "module-xt-sync.html",
                      "tf": 1.7241379310344827
                    }
                  }
                }
              }
            },
            "g": {
              "docs": {},
              "r": {
                "docs": {},
                "a": {
                  "docs": {},
                  "d": {
                    "docs": {
                      "module-xt-sync.html": {
                        "ref": "module-xt-sync.html",
                        "tf": 5.172413793103448
                      }
                    }
                  }
                }
              }
            }
          },
          "t": {
            "docs": {},
            "i": {
              "docs": {},
              "l": {
                "docs": {
                  "index.html": {
                    "ref": "index.html",
                    "tf": 4.117647058823529
                  }
                }
              }
            }
          },
          "s": {
            "docs": {
              "undefined": {
                "tf": 11.666666666666666
              },
              "list_tutorial.html": {
                "ref": "list_tutorial.html",
                "tf": 17.5
              },
              "module-xt-create.html": {
                "ref": "module-xt-create.html",
                "tf": 6.25
              },
              "module-xt-docs.html": {
                "ref": "module-xt-docs.html",
                "tf": 2.1739130434782608
              },
              "module-xt-test.html": {
                "ref": "module-xt-test.html",
                "tf": 2.898550724637681
              }
            }
          },
          "n": {
            "docs": {},
            "i": {
              "docs": {},
              "t": {
                "docs": {
                  "undefined": {
                    "tf": 23.333333333333332
                  },
                  "module-xt-test.html": {
                    "ref": "module-xt-test.html",
                    "tf": 1.4492753623188406
                  }
                }
              }
            }
          }
        },
        "w": {
          "docs": {},
          "a": {
            "docs": {},
            "y": {
              "docs": {
                "index.html": {
                  "ref": "index.html",
                  "tf": 4.117647058823529
                }
              }
            },
            "n": {
              "docs": {},
              "t": {
                "docs": {
                  "module-xt-docs.html": {
                    "ref": "module-xt-docs.html",
                    "tf": 0.7246376811594203
                  }
                }
              }
            }
          },
          "e": {
            "docs": {},
            "b": {
              "docs": {
                "module-xt-build.html": {
                  "ref": "module-xt-build.html",
                  "tf": 2.1739130434782608
                }
              }
            }
          },
          "i": {
            "docs": {},
            "n": {
              "docs": {},
              "d": {
                "docs": {},
                "o": {
                  "docs": {},
                  "w": {
                    "docs": {
                      "module-xt-test.html": {
                        "ref": "module-xt-test.html",
                        "tf": 1.4492753623188406
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "g": {
          "docs": {},
          "l": {
            "docs": {},
            "o": {
              "docs": {},
              "b": {
                "docs": {},
                "a": {
                  "docs": {},
                  "l": {
                    "docs": {
                      "global.html": {
                        "ref": "global.html",
                        "tf": 2045
                      },
                      "module-xt-test.html": {
                        "ref": "module-xt-test.html",
                        "tf": 0.7246376811594203
                      }
                    }
                  }
                }
              }
            }
          },
          "e": {
            "docs": {},
            "t": {
              "docs": {
                "undefined": {
                  "tf": 950
                }
              }
            },
            "n": {
              "docs": {},
              "e": {
                "docs": {},
                "r": {
                  "docs": {
                    "undefined": {
                      "tf": 14
                    },
                    "module-xt-build.html": {
                      "ref": "module-xt-build.html",
                      "tf": 2.1739130434782608
                    },
                    "module-xt-docs.html": {
                      "ref": "module-xt-docs.html",
                      "tf": 0.7246376811594203
                    }
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "i": {
              "docs": {},
              "d": {
                "docs": {
                  "undefined": {
                    "tf": 25
                  },
                  "list_tutorial.html": {
                    "ref": "list_tutorial.html",
                    "tf": 600
                  }
                }
              }
            }
          },
          "i": {
            "docs": {},
            "t": {
              "docs": {},
              "i": {
                "docs": {},
                "g": {
                  "docs": {},
                  "n": {
                    "docs": {},
                    "o": {
                      "docs": {},
                      "r": {
                        "docs": {
                          "module-xt-clean.html": {
                            "ref": "module-xt-clean.html",
                            "tf": 1.9230769230769231
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "h": {
          "docs": {},
          "e": {
            "docs": {},
            "r": {
              "docs": {},
              "e": {
                "docs": {
                  "undefined": {
                    "tf": 11.666666666666666
                  },
                  "module-xt-docs.html": {
                    "ref": "module-xt-docs.html",
                    "tf": 0.7246376811594203
                  }
                }
              }
            },
            "l": {
              "docs": {},
              "p": {
                "docs": {
                  "undefined": {
                    "tf": 950
                  }
                }
              }
            },
            "a": {
              "docs": {},
              "v": {
                "docs": {},
                "i": {
                  "docs": {},
                  "l": {
                    "docs": {},
                    "i": {
                      "docs": {
                        "module-xt-sync.html": {
                          "ref": "module-xt-sync.html",
                          "tf": 1.7241379310344827
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "v": {
          "docs": {},
          "e": {
            "docs": {},
            "r": {
              "docs": {},
              "s": {
                "docs": {},
                "i": {
                  "docs": {},
                  "o": {
                    "docs": {},
                    "n": {
                      "docs": {
                        "undefined": {
                          "tf": 14
                        },
                        "module-xt-sync.html": {
                          "ref": "module-xt-sync.html",
                          "tf": 1.7241379310344827
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "s": {
            "docs": {},
            "c": {
              "docs": {},
              "o": {
                "docs": {},
                "d": {
                  "docs": {},
                  "e": {
                    "docs": {
                      "module-xt-clean.html": {
                        "ref": "module-xt-clean.html",
                        "tf": 1.9230769230769231
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "x": {
          "docs": {},
          "t": {
            "docs": {
              "undefined": {
                "tf": 950
              },
              "module-xt-build.html": {
                "ref": "module-xt-build.html",
                "tf": 300
              },
              "module-xt-clean.html": {
                "ref": "module-xt-clean.html",
                "tf": 300
              },
              "module-xt-create.html": {
                "ref": "module-xt-create.html",
                "tf": 300
              },
              "module-xt-docs.html": {
                "ref": "module-xt-docs.html",
                "tf": 300
              },
              "module-xt-sync.html": {
                "ref": "module-xt-sync.html",
                "tf": 300
              },
              "module-xt-test.html": {
                "ref": "module-xt-test.html",
                "tf": 300
              }
            },
            "d": {
              "docs": {},
              "o": {
                "docs": {},
                "c": {
                  "docs": {
                    "module-xt-docs.html": {
                      "ref": "module-xt-docs.html",
                      "tf": 0.7246376811594203
                    }
                  },
                  "s": {
                    "docs": {},
                    ".": {
                      "docs": {},
                      "j": {
                        "docs": {},
                        "s": {
                          "docs": {},
                          "o": {
                            "docs": {},
                            "n": {
                              "docs": {
                                "module-xt-docs.html": {
                                  "ref": "module-xt-docs.html",
                                  "tf": 0.7246376811594203
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "f": {
          "docs": {},
          "i": {
            "docs": {},
            "l": {
              "docs": {},
              "e": {
                "docs": {
                  "undefined": {
                    "tf": 17.5
                  },
                  "module-xt-build.html": {
                    "ref": "module-xt-build.html",
                    "tf": 2.1739130434782608
                  },
                  "module-xt-clean.html": {
                    "ref": "module-xt-clean.html",
                    "tf": 5.769230769230769
                  },
                  "module-xt-docs.html": {
                    "ref": "module-xt-docs.html",
                    "tf": 2.898550724637681
                  },
                  "module-xt-sync.html": {
                    "ref": "module-xt-sync.html",
                    "tf": 5.172413793103448
                  }
                }
              }
            }
          },
          "l": {
            "docs": {},
            "a": {
              "docs": {},
              "g": {
                "docs": {
                  "module-xt-build.html": {
                    "ref": "module-xt-build.html",
                    "tf": 2.1739130434782608
                  },
                  "module-xt-clean.html": {
                    "ref": "module-xt-clean.html",
                    "tf": 1.9230769230769231
                  },
                  "module-xt-docs.html": {
                    "ref": "module-xt-docs.html",
                    "tf": 0.7246376811594203
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "o": {
              "docs": {},
              "d": {
                "docs": {},
                "o": {
                  "docs": {},
                  "c": {
                    "docs": {
                      "module-xt-docs.html": {
                        "ref": "module-xt-docs.html",
                        "tf": 0.7246376811594203
                      }
                    }
                  }
                }
              }
            },
            "l": {
              "docs": {},
              "l": {
                "docs": {},
                "o": {
                  "docs": {},
                  "w": {
                    "docs": {
                      "module-xt-test.html": {
                        "ref": "module-xt-test.html",
                        "tf": 0.7246376811594203
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "n": {
          "docs": {},
          "o": {
            "docs": {},
            "t": {
              "docs": {},
              "e": {
                "docs": {
                  "undefined": {
                    "tf": 35
                  }
                }
              }
            },
            "d": {
              "docs": {},
              "e": {
                "docs": {},
                "_": {
                  "docs": {},
                  "m": {
                    "docs": {},
                    "o": {
                      "docs": {},
                      "d": {
                        "docs": {},
                        "u": {
                          "docs": {},
                          "l": {
                            "docs": {
                              "module-xt-clean.html": {
                                "ref": "module-xt-clean.html",
                                "tf": 1.9230769230769231
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "e": {
            "docs": {},
            "w": {
              "docs": {
                "module-xt-create.html": {
                  "ref": "module-xt-create.html",
                  "tf": 6.25
                }
              }
            }
          },
          "a": {
            "docs": {},
            "m": {
              "docs": {},
              "e": {
                "docs": {
                  "module-xt-docs.html": {
                    "ref": "module-xt-docs.html",
                    "tf": 0.7246376811594203
                  }
                }
              }
            }
          },
          "y": {
            "docs": {},
            "c": {
              "docs": {
                "module-xt-test.html": {
                  "ref": "module-xt-test.html",
                  "tf": 0.7246376811594203
                }
              }
            }
          }
        },
        "o": {
          "docs": {},
          "l": {
            "docs": {},
            "d": {
              "docs": {},
              "_": {
                "docs": {},
                "r": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "a": {
                      "docs": {},
                      "d": {
                        "docs": {},
                        "m": {
                          "docs": {
                            "undefined": {
                              "tf": 1900
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "n": {
            "docs": {
              "module-xt-clean.html": {
                "ref": "module-xt-clean.html",
                "tf": 1.9230769230769231
              }
            },
            "c": {
              "docs": {
                "module-xt-create.html": {
                  "ref": "module-xt-create.html",
                  "tf": 6.25
                }
              }
            }
          },
          "p": {
            "docs": {},
            "e": {
              "docs": {},
              "r": {
                "docs": {
                  "module-xt-clean.html": {
                    "ref": "module-xt-clean.html",
                    "tf": 1.9230769230769231
                  }
                }
              }
            },
            "t": {
              "docs": {},
              "i": {
                "docs": {},
                "o": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "module-xt-docs.html": {
                        "ref": "module-xt-docs.html",
                        "tf": 1.4492753623188406
                      }
                    }
                  }
                }
              },
              "s": {
                "docs": {},
                ".": {
                  "docs": {},
                  "t": {
                    "docs": {},
                    "e": {
                      "docs": {},
                      "m": {
                        "docs": {},
                        "p": {
                          "docs": {},
                          "l": {
                            "docs": {
                              "module-xt-docs.html": {
                                "ref": "module-xt-docs.html",
                                "tf": 0.7246376811594203
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "v": {
            "docs": {},
            "e": {
              "docs": {},
              "r": {
                "docs": {
                  "module-xt-clean.html": {
                    "ref": "module-xt-clean.html",
                    "tf": 1.9230769230769231
                  }
                },
                "r": {
                  "docs": {},
                  "i": {
                    "docs": {},
                    "d": {
                      "docs": {
                        "module-xt-docs.html": {
                          "ref": "module-xt-docs.html",
                          "tf": 0.7246376811594203
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "m": {
          "docs": {},
          "i": {
            "docs": {},
            "n": {
              "docs": {},
              "i": {
                "docs": {},
                "f": {
                  "docs": {},
                  "i": {
                    "docs": {
                      "module-xt-build.html": {
                        "ref": "module-xt-build.html",
                        "tf": 2.1739130434782608
                      }
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "d": {
              "docs": {},
              "u": {
                "docs": {},
                "l": {
                  "docs": {
                    "module-xt-build.html": {
                      "ref": "module-xt-build.html",
                      "tf": 110
                    },
                    "module-xt-clean.html": {
                      "ref": "module-xt-clean.html",
                      "tf": 110
                    },
                    "module-xt-create.html": {
                      "ref": "module-xt-create.html",
                      "tf": 110
                    },
                    "module-xt-docs.html": {
                      "ref": "module-xt-docs.html",
                      "tf": 110
                    },
                    "module-xt-sync.html": {
                      "ref": "module-xt-sync.html",
                      "tf": 110
                    },
                    "module-xt-test.html": {
                      "ref": "module-xt-test.html",
                      "tf": 110.72463768115942
                    }
                  },
                  "e": {
                    "docs": {},
                    ":": {
                      "docs": {},
                      "x": {
                        "docs": {},
                        "t": {
                          "docs": {
                            "module-xt-build.html": {
                              "ref": "module-xt-build.html",
                              "tf": 650
                            },
                            "module-xt-clean.html": {
                              "ref": "module-xt-clean.html",
                              "tf": 650
                            },
                            "module-xt-create.html": {
                              "ref": "module-xt-create.html",
                              "tf": 650
                            },
                            "module-xt-docs.html": {
                              "ref": "module-xt-docs.html",
                              "tf": 650
                            },
                            "module-xt-sync.html": {
                              "ref": "module-xt-sync.html",
                              "tf": 650
                            },
                            "module-xt-test.html": {
                              "ref": "module-xt-test.html",
                              "tf": 650
                            }
                          }
                        }
                      }
                    }
                  }
                }
              },
              "i": {
                "docs": {},
                "f": {
                  "docs": {},
                  "i": {
                    "docs": {
                      "module-xt-sync.html": {
                        "ref": "module-xt-sync.html",
                        "tf": 1.7241379310344827
                      }
                    }
                  }
                }
              }
            },
            "r": {
              "docs": {},
              "e": {
                "docs": {
                  "module-xt-docs.html": {
                    "ref": "module-xt-docs.html",
                    "tf": 0.7246376811594203
                  }
                }
              }
            },
            "c": {
              "docs": {},
              "h": {
                "docs": {},
                "a": {
                  "docs": {
                    "module-xt-test.html": {
                      "ref": "module-xt-test.html",
                      "tf": 1.4492753623188406
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "n": {
              "docs": {},
              "n": {
                "docs": {},
                "e": {
                  "docs": {},
                  "r": {
                    "docs": {
                      "module-xt-sync.html": {
                        "ref": "module-xt-sync.html",
                        "tf": 1.7241379310344827
                      }
                    }
                  }
                }
              },
              "u": {
                "docs": {},
                "a": {
                  "docs": {},
                  "l": {
                    "docs": {
                      "module-xt-sync.html": {
                        "ref": "module-xt-sync.html",
                        "tf": 1.7241379310344827
                      }
                    }
                  }
                }
              }
            }
          },
          "e": {
            "docs": {},
            "a": {
              "docs": {},
              "n": {
                "docs": {
                  "module-xt-test.html": {
                    "ref": "module-xt-test.html",
                    "tf": 0.7246376811594203
                  }
                }
              }
            }
          }
        },
        "a": {
          "docs": {},
          "d": {
            "docs": {},
            "d": {
              "docs": {
                "module-xt-docs.html": {
                  "ref": "module-xt-docs.html",
                  "tf": 0.7246376811594203
                }
              }
            },
            "v": {
              "docs": {},
              "i": {
                "docs": {},
                "s": {
                  "docs": {
                    "module-xt-sync.html": {
                      "ref": "module-xt-sync.html",
                      "tf": 1.7241379310344827
                    }
                  }
                }
              }
            }
          },
          "l": {
            "docs": {},
            "t": {
              "docs": {},
              "e": {
                "docs": {},
                "r": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "module-xt-docs.html": {
                        "ref": "module-xt-docs.html",
                        "tf": 0.7246376811594203
                      }
                    }
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "t": {
              "docs": {},
              "o": {
                "docs": {},
                "m": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "module-xt-docs.html": {
                          "ref": "module-xt-docs.html",
                          "tf": 0.7246376811594203
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "p": {
            "docs": {},
            "i": {
              "docs": {
                "module-xt-test.html": {
                  "ref": "module-xt-test.html",
                  "tf": 0.7246376811594203
                }
              }
            }
          },
          "r": {
            "docs": {},
            "r": {
              "docs": {},
              "o": {
                "docs": {},
                "w": {
                  "docs": {
                    "module-xt-test.html": {
                      "ref": "module-xt-test.html",
                      "tf": 0.7246376811594203
                    }
                  }
                }
              }
            }
          }
        },
        "j": {
          "docs": {},
          "s": {
            "docs": {},
            "d": {
              "docs": {},
              "o": {
                "docs": {},
                "c": {
                  "docs": {
                    "module-xt-docs.html": {
                      "ref": "module-xt-docs.html",
                      "tf": 2.1739130434782608
                    }
                  }
                },
                "m": {
                  "docs": {
                    "module-xt-test.html": {
                      "ref": "module-xt-test.html",
                      "tf": 0.7246376811594203
                    }
                  }
                }
              }
            }
          }
        },
        "k": {
          "docs": {},
          "e": {
            "docs": {},
            "y": {
              "docs": {
                "module-xt-docs.html": {
                  "ref": "module-xt-docs.html",
                  "tf": 0.7246376811594203
                }
              }
            }
          }
        }
      },
      "length": 277
    },
    "corpusTokens": [
      "add",
      "advis",
      "altern",
      "api",
      "arrow",
      "automat",
      "babel",
      "base",
      "browser",
      "build",
      "c",
      "call",
      "chai",
      "chang",
      "changelog",
      "chrome",
      "class",
      "clean",
      "cli",
      "code",
      "command",
      "compil",
      "comput",
      "config",
      "configur",
      "cool",
      "coverag",
      "creat",
      "debug",
      "default",
      "defin",
      "delet",
      "detail",
      "dev",
      "develop",
      "directori",
      "dist",
      "distribut",
      "doc",
      "document",
      "e",
      "each",
      "env",
      "environ",
      "es6",
      "except",
      "execut",
      "expect",
      "explicitli",
      "extend",
      "extens",
      "file",
      "flag",
      "follow",
      "foodoc",
      "gener",
      "get",
      "gitignor",
      "global",
      "guid",
      "heavili",
      "help",
      "here",
      "idea",
      "ignor",
      "implement",
      "import",
      "includ",
      "index",
      "initi",
      "instal",
      "instead",
      "iter",
      "jsdoc",
      "jsdom",
      "key",
      "latest",
      "learn",
      "line",
      "link",
      "list",
      "list:tutori",
      "locat",
      "look",
      "manner",
      "manual",
      "mean",
      "minifi",
      "mocha",
      "modifi",
      "modul",
      "module:xt",
      "more",
      "name",
      "new",
      "node_modul",
      "note",
      "nyc",
      "old_readm",
      "on",
      "onc",
      "oper",
      "option",
      "opts.templ",
      "over",
      "overrid",
      "package.json",
      "pass",
      "path",
      "plan",
      "pre",
      "prod",
      "product",
      "project",
      "promis",
      "provid",
      "purpos",
      "readm",
      "releas",
      "release.zip",
      "remov",
      "resourc",
      "root",
      "run",
      "see",
      "separ",
      "setup",
      "showcas",
      "simpli",
      "singl",
      "sinon",
      "sourc",
      "speed",
      "stale",
      "start",
      "store",
      "such",
      "suitabl",
      "sync",
      "synchron",
      "syntax",
      "systemat",
      "templat",
      "test",
      "theme",
      "tool",
      "tutori",
      "unit",
      "up",
      "updat",
      "upgrad",
      "upload",
      "us",
      "util",
      "version",
      "vscode",
      "want",
      "way",
      "web",
      "window",
      "xt",
      "xtdoc",
      "xtdocs.json"
    ],
    "pipeline": [
      "trimmer",
      "stopWordFilter",
      "stemmer"
    ]
  },
  "store": {
    "index.html": {
      "id": "index.html",
      "kind": "readme",
      "title": "Extension CLI",
      "longname": "index",
      "name": "Extension CLI",
      "tags": "index",
      "summary": "Command-line utility tool that speeds up chrome extension development by providing a systematic way to build, test and document extension projects.",
      "description": "",
      "body": ""
    },
    "global.html": {
      "id": "global.html",
      "kind": "global",
      "title": "Globals",
      "longname": "global",
      "name": "Globals",
      "tags": "global",
      "summary": "All documented globals.",
      "description": "",
      "body": ""
    },
    "undefined": {
      "kind": "tutorial",
      "title": "old_README",
      "longname": "old_README",
      "name": "old_README",
      "tags": "old_README",
      "summary": "",
      "description": "",
      "body": ""
    },
    "list_tutorial.html": {
      "id": "list_tutorial.html",
      "kind": "list",
      "title": "Guide",
      "longname": "list:tutorial",
      "name": "Guide",
      "tags": "list:tutorial",
      "summary": "Learn how to use extension CLI",
      "description": "",
      "body": ""
    },
    "module-xt-build.html": {
      "id": "module-xt-build.html",
      "kind": "module",
      "title": "xt-build",
      "longname": "module:xt-build",
      "name": "xt-build",
      "tags": "module:xt-build",
      "summary": "",
      "description": "Build command generates a dist/ directory that can be debugged in the browser. When called with production env flag, -e prod, this command will minify and compile a release.zip file that can be uploaded to Chrome Web Store for distribution.",
      "body": ""
    },
    "module-xt-clean.html": {
      "id": "module-xt-clean.html",
      "kind": "module",
      "title": "xt-clean",
      "longname": "module:xt-clean",
      "name": "xt-clean",
      "tags": "module:xt-clean",
      "summary": "",
      "description": "Clean operation iterates over files and directories listed in the project .gitignore file, and removes all ignored files and directories, except node_modules, .idea/, and .vscode. To remove these directories, you must explicitly pass a flag to delete each one of them.",
      "body": ""
    },
    "module-xt-create.html": {
      "id": "module-xt-create.html",
      "kind": "module",
      "title": "xt-create",
      "longname": "module:xt-create",
      "name": "xt-create",
      "tags": "module:xt-create",
      "summary": "",
      "description": "Once implemented, this command can be used to create a new extension project.",
      "body": ""
    },
    "module-xt-docs.html": {
      "id": "module-xt-docs.html",
      "kind": "module",
      "title": "xt-docs",
      "longname": "module:xt-docs",
      "name": "xt-docs",
      "tags": "module:xt-docs",
      "summary": "",
      "description": "Docs command generates documentation for the project. This command uses jsdocs syntax. See About JSDoc for more details, including configuration options here. The default template for the docs is FooDoc. You can override this theme in the project by changing opts.template in jsdoc config file. By default, this command will automatically look for configuration in the project package.json. Use \"xtdocs\" key to define config options in package.json. Alternatively add a separate configuration file .xtdocs.json in the project root; or explicitly provide a path to a config file. If you want to define configuration in some other location, use -c/--config flag to provide path and name of the configuration file.",
      "body": ""
    },
    "module-xt-sync.html": {
      "id": "module-xt-sync.html",
      "kind": "module",
      "title": "xt-sync",
      "longname": "module:xt-sync",
      "name": "xt-sync",
      "tags": "module:xt-sync",
      "summary": "",
      "description": "The purpose of this command is to upgrade configuration files of a stale project to latest version, where this CLI tool will provide updated project configuration files. If the config files have been modified heavily for the project, it is not advisable to upgrade them in this manner. Instead you should upgrade such configs manually.",
      "body": ""
    },
    "module-xt-test.html": {
      "id": "module-xt-test.html",
      "kind": "module",
      "title": "xt-test",
      "longname": "module:xt-test",
      "name": "xt-test",
      "tags": "module:xt-test",
      "summary": "",
      "description": "This command will run project unit tests located in test/ directory. This command will setup extension testing environment that is pre-initialized with mocha, chai (including chai-as-promised) and expect. nyc is used for computing code coverage. The following browser APIs are also initialized: window, chrome. Window is setup using jsdom-global and chrome using sinon-chrome. Mocha will execute with babel, meaning you can use this test environment to test ES6 modules with imports, arrows, and classes. You may extend this test environment with a single project; this is simply the base setup for running unit tests. Or you may create your own test environment if this is not suitable.",
      "body": ""
    }
  }
};